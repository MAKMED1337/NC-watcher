...
async def main():
	...
	async with p:
		...
		task_info = TaskInfo(list_info, {'resubmits': 0, 'reward': 0, 'reviews': [{'mine': True, 'comment': 'TEST'}], 'comment': 'IDK'})
		...
		async with db.transaction():
			...
			await p.notify_payment(r, (await UnpaidRewards.get('TEST', ActionEnum.review))[0]._mapping)
		...
		async with db.transaction():
			...
			await p.notify_payment(r, (await UnpaidRewards.get('TEST', ActionEnum.task))[0]._mapping)
			await UnpaidRewards.clear('TEST')
