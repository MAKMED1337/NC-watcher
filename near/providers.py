...
class JsonProvider(object):
	...
	async def json_rpc(self, method: str, params: Union[dict, list, str], timeout: aiohttp.ClientTimeout = None) -> dict:
		j = {
			'method': method,
			'params': params,
			'id': 'dontcare',
	...
	async def get_validators(self, timeout: aiohttp.ClientTimeout = None) -> dict:
		return await self.json_rpc('validators', [None], timeout)

	async def query(self, query_object: Union[dict, list, str], timeout: aiohttp.ClientTimeout = None) -> dict:
	...
	async def get_chunk(self, chunk_id: str, timeout: aiohttp.ClientTimeout = None) -> dict:
		return await self.json_rpc('chunk', {'chunk_id': chunk_id}, timeout)

	async def get_tx(self, tx_hash: str, tx_recipient_id: str, timeout: aiohttp.ClientTimeout = None) -> dict:
	...
	async def get_validators_ordered(self, block_hash: bytes, timeout: aiohttp.ClientTimeout = None) -> dict:
	...
	async def get_next_light_client_block(self, last_block_hash, timeout: aiohttp.ClientTimeout = None) -> dict:
		return await self.json_rpc('next_light_client_block', [last_block_hash], timeout)

	async def get_receipt(self, receipt_hash, timeout: aiohttp.ClientTimeout = None) -> dict:
		return await self.json_rpc('EXPERIMENTAL_receipt', [receipt_hash], timeout)

	async def get_tx_receipts(self, tx_hash, sender_id, timeout: aiohttp.ClientTimeout = None):
