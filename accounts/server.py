...
async def lock_account(account: NearCrowdAccount) -> NearCrowdAccount:
...
def unlock(accounts: list[NearCrowdAccount]) -> None:
    for i in accounts:
...
class ConnectionHandler:
    ...
    async def proceed_client(self) -> None:
        tasks = []
    ...
    async def _proceed_call(self, resp: Response) -> None:
    ...
    async def execute_call(self, call: FuncCall) -> Any:
        ...
        return {k.account_id: v for k, v in zip(self.accounts, result, strict=True)}
