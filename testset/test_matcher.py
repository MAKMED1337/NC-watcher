from watcher.payments_matcher import match_payments
...
def is_matching(actions: list[IAction], rewards: list[UnpaidRewards]) -> bool:
	return match_payments(actions, rewards) == list(zip(actions, rewards))

def test_matcher_empty():
	assert match_payments([], []) == [] #match

def test_matcher_different_size():
	assert match_payments([], [UnpaidRewards()]) is None
	assert match_payments([IAction()], []) is None
	assert match_payments([IAction(), IAction()], [UnpaidRewards()]) is None
	assert match_payments([IAction()], [UnpaidRewards(), UnpaidRewards()]) is None
...
async def test_matcher_task_1_bad():
	...
	assert match_payments([task], [UnpaidRewards(cost=611, coef=1.1, action=ActionEnum.task, adjustment=1)]) is None
	assert match_payments([task], [UnpaidRewards(cost=0, coef=1.1, action=ActionEnum.task, adjustment=0)]) is None
	assert match_payments([task], [UnpaidRewards(cost=1000, coef=1.1, action=ActionEnum.task, adjustment=1)]) is None
...
async def test_matcher_review_1_bad():
	...
	assert match_payments([review], [reward]) is None
...
async def test_matcher_review_2_bad():
	...
	review2 = await create_review(18, 0, 0, 0, 400)
	reward2 = UnpaidRewards(cost=440, coef=1.1, action=ActionEnum.review, adjustment=1)
	assert match_payments([review1, review2], [reward1, reward2]) is None
